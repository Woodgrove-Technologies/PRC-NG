...
<p>The next step in producing this result are to enhance the performance of the current DexiNed stream we have achieved. There are several ways to approach this task with the most signifacen change occuring when the program can switch from the CPU to the iGPU. This posed challenges on my attempt, however I found some advice/important places to look. In the docker container log there is information about the model server and device. When starting up, a line of interest reads: "available devices: CPU, GNA. GNA is the Gaussian Neural Accelerator which does not apply the performance we are looking for. Look into ways to have this line detect the iGPU inside the NUC.</p>
<p>Another important next step is to adjust the frame size expected by the model. The current model provided in this document is expecting 352x352 pixels which is below what Woodgrove's clients expect since it leads to less accurate information about the image. This may require finding/changing the .onnx model to expect a different shaped input. Once these are completed, begin running the DexiNed file written by Woodgrove which can be found on the computer in the warehouse.</p>
<hr>
